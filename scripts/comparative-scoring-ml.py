...
def init_scores(tree,scores,genome_id2target_id):
    ...
    for clade in tree_used.traverse(strategy='postorder'):
        if clade.is_leaf():
            ...
            lbl = max(lchild.dist,EPS) # avoid division by zero
            rbl = max(rchild.dist,EPS)
...
def main():
    ...
    for query_id in scores:
        ...
        if n_genomes < 3:
            logger.info(f"skip {query_id} due to small number of homologs: {n_genomes} present.")
            continue
